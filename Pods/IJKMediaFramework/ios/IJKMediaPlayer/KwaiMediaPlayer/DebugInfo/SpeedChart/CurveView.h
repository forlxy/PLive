...
extern NSString* const CurveViewPointValuesRowValueKey;
extern NSString* const CurveViewPointValuesColumnValueKey;

@protocol CurveViewDelegate <NSObject>

@optional
/**手势开始时调用,回调手指所在位置的最近的坐标和该坐标对应的传入的字典值[注意：该坐标是相对于CurveView的，如要使用请使用convert方法进行转换(如:[self.view
 * convertPoint:location fromView:view])]*/
- (void)curveView:(CurveView*)view
    longGestureBeganAtcurrentLocation:(CGPoint)location
                                value:(NSDictionary*)value;
/**手势移动时调用,回调手指所在位置的最近的坐标和对应的值*/
- (void)curveView:(CurveView*)view
    LongGestureChangedAtcurrentLocation:(CGPoint)location
                                  value:(NSDictionary*)value;
/**手势结束调用*/
- (void)curveViewLongGestureEnd:(CurveView*)view;
...
#pragma 数据相关属性
//所有的坐标点数组，需要传递字典数组，字典必须包含key值为
// CurveViewPointValuesRowValueKey 和 CurveViewPointValuesColumnValueKey
//分别代表横纵的值,可以包含其他值，在代理中可回调，必传
