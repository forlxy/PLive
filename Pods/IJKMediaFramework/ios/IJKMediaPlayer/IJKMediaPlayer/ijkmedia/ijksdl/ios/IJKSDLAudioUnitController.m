...
- (id)initWithAudioSpec:(const SDL_AudioSpec*)aSpec {
    ...
    if (self) {
        ...
        UInt32 flag = 1;
        status = AudioUnitSetProperty(auUnit, kAudioOutputUnitProperty_EnableIO,
        ...
        status = AudioUnitSetProperty(auUnit, kAudioUnitProperty_StreamFormat,
                                      kAudioUnitScope_Input, 0, &streamDescription, i_param_size);
        if (status != noErr) {
        ...
        status = AudioUnitGetProperty(auUnit, kAudioUnitProperty_StreamFormat,
                                      kAudioUnitScope_Input, 0, &streamDescription, &i_param_size);
        if (status != noErr) {
        ...
        status = AudioUnitSetProperty(auUnit, kAudioUnitProperty_SetRenderCallback,
...
- (void)play {
    ...
    if (status != noErr) NSLog(@"AudioUnit: AudioOutputUnitStart failed (%d)\n", (int)status);
}
...
- (void)pause {
    ...
    if (status != noErr) ALOGE("AudioUnit: failed to stop AudioUnit (%d)\n", (int)status);
}
...
- (void)stop {
    ...
    OSStatus status = AudioOutputUnitStop(_auUnit);
    if (status != noErr) ALOGE("AudioUnit: failed to stop AudioUnit (%d)", (int)status);
}
...
- (void)close {
    ...
    AudioUnitSetProperty(_auUnit, kAudioUnitProperty_SetRenderCallback, kAudioUnitScope_Input, 0,
...
static OSStatus RenderCallback(void* inRefCon, AudioUnitRenderActionFlags* ioActionFlags,
    ...
    @autoreleasepool {
        ...
        for (int i = 0; i < (int)ioData->mNumberBuffers; i++) {
            AudioBuffer* ioBuffer = &ioData->mBuffers[i];
            (*auController.spec.callback)(auController.spec.userdata, ioBuffer->mData,
                                          ioBuffer->mDataByteSize);
        }
