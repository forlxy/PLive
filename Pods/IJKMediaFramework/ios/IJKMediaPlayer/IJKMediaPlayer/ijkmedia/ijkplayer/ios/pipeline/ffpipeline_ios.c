...
static IJKFF_Pipenode* func_open_video_decoder(IJKFF_Pipeline* pipeline, FFPlayer* ffp) {
    ...
    int codec = ffp->is->viddec.avctx->codec_id;
...
static SDL_Aout* func_open_audio_output(IJKFF_Pipeline* pipeline, FFPlayer* ffp) {
    SDL_Aout* aout = SDL_AoutIos_CreateForAudioUnit(pipeline->extra);
    if(aout) {
        aout->ios.live_event_cb = ffp->live_event_callback;
    }
    return aout;
}
