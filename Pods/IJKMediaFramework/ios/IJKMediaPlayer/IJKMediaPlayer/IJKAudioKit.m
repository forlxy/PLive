...
- (void)setupAudioSession {
    ...
    if (![AVAudioSessionCategoryPlayAndRecord
            isEqualToString:[AVAudioSession sharedInstance].category] &&
        !
        [AVAudioSessionCategoryPlayback isEqualToString:[AVAudioSession sharedInstance].category]) {
        if (NO == [[AVAudioSession sharedInstance] setCategory:AVAudioSessionCategoryPlayback
                                                         error:&error]) {
    ...
    if (_enableMixAudio) {
        ...
        [session setCategory:category withOptions:opts error:nil];
    }
...
- (void)handleInterruption:(NSNotification*)notification {
    ...
    switch (reason) {
        case AVAudioSessionInterruptionTypeBegan: {
            NSLog(@"AVAudioSessionInterruptionType Began\n");
            [self setActive:NO];
            break;
        }
        case AVAudioSessionInterruptionTypeEnded: {
            NSLog(@"AVAudioSessionInterruptionType  Ended\n");
            [self setActive:YES];
            break;
        }
    }
