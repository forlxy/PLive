...
+ (IJKFFOptions*)optionsByDefault {
    ...
    //    [options setPlayerOptionIntValue:960
    //    forKey:@"videotoolbox-max-frame-width"];

    //    [options setFormatOptionIntValue:0 forKey:@"auto_convert"]; [options
    //    setFormatOptionIntValue:1                  forKey:@"reconnect"];
...
- (void)applyTo:(IjkMediaPlayer*)mediaPlayer {
    [_optionCategories
        enumerateKeysAndObjectsUsingBlock:^(id categoryKey, id categoryDict, BOOL* stopOuter) {
            [categoryDict enumerateKeysAndObjectsUsingBlock:^(id optKey, id optValue, BOOL* stop) {
                if ([optValue isKindOfClass:[NSNumber class]]) {
                    ijkmp_set_option_int(mediaPlayer, (int)[categoryKey integerValue],
                                         [optKey UTF8String], [optValue longLongValue]);
                } else if ([optValue isKindOfClass:[NSString class]]) {
                    ijkmp_set_option(mediaPlayer, (int)[categoryKey integerValue],
                                     [optKey UTF8String], [optValue UTF8String]);
                }
            }];
        }];
}
