...
class DefaultHttpDataSource final : public HttpDataSource, public HasConnectionInfo {
  public:
    DefaultHttpDataSource(std::shared_ptr<DownloadManager> download_manager,
                          std::shared_ptr<TransferListener<HttpDataSource> > listener,
                          const DownloadOpts& opts,
                          AwesomeCacheRuntimeInfo* ac_rt_info);

    virtual ~DefaultHttpDataSource();
