...
class OfflineCachePreloadLister : public CacheSessionListener {
  public:
    OfflineCachePreloadLister(TaskListener* listener);
    ~OfflineCachePreloadLister();
...
class OfflineCachePreloadTask : public Task {
  public:

    OfflineCachePreloadTask(std::unique_ptr<DataSourceSeekable> data_source,
                            const DataSpec& spec,
                            DataSourceType type,
                            TaskListener* listener,
                            int64_t pos,
                            int64_t len);
    virtual ~OfflineCachePreloadTask();
