...
void SDL_WaitThread(SDL_Thread* thread, int* status) {
    if (!thread) {
        ALOGE("[SDL_WaitThread] thread is NULL! return");
        return;
    }

    ALOGI("[SDL_WaitThread][Start], thread:%s, id:%ld", thread->name, thread->id);
    int ret = pthread_join(thread->id, NULL);
    ALOGI("[SDL_WaitThread][End  ], thread:%s, ret:%d", thread->name, ret);
