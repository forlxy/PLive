...
static EGLBoolean IJK_EGL_prepareRenderer(IJK_EGL* egl, SDL_VoutOverlay* overlay) {
    ...
    if (!IJK_GLES2_Renderer_isValid(opaque->renderer) ||
        ...
        if (opaque->video_scaling_mode > 0)
            IJK_GLES2_Renderer_setGravity(opaque->renderer, opaque->video_scaling_mode, opaque->window_width, opaque->window_height);

        if (opaque->rotate_degree > 0)
            IJK_GLES2_Renderer_setRotateDegress(opaque->renderer, opaque->rotate_degree);
    }
...
void IJK_EGL_scale(IJK_EGL* egl, int gravity, int width, int height) {
    IJK_EGL_Opaque* opaque = egl->opaque;
    if (!opaque)
        return;

    if (opaque->renderer != NULL) {
        opaque->video_scaling_mode = gravity;
        opaque->window_width = width;
        opaque->window_height = height;
        IJK_GLES2_Renderer_setGravity(opaque->renderer, gravity, width, height);
    } else {
        opaque->video_scaling_mode = gravity;
        opaque->window_width = width;
        opaque->window_height = height;
...
EGLBoolean  IJK_EGL_set_degree(IJK_EGL* egl, int degree) {
    ...
    if (opaque->renderer != NULL) {
        opaque->rotate_degree = degree;
        ret = IJK_GLES2_Renderer_setRotateDegress(opaque->renderer, degree);
    }
...
static NV21RenderType g_nv21RenderType = NV21_RENDER_TYPE_UNDEFINED;

// return render type "GLES" or "NativeWindow" for NV21 renderer
// only Qualcomm's Adreno GPUs use "NativeWindow" for better performance
// other GPUs use "GLES" as a generic solution
NV21RenderType IJK_EGL_get_nv21_render_type(IJK_EGL* egl, EGLNativeWindowType window) {
    if (g_nv21RenderType != NV21_RENDER_TYPE_UNDEFINED)
        return g_nv21RenderType;

    g_nv21RenderType = NV21_RENDER_TYPE_GLES;
    if (!egl)
        return g_nv21RenderType;
    ...
    if (gpuName && strlen(gpuName) >= 6 && strncmp(gpuName, "Adreno", 6) == 0)
        g_nv21RenderType = NV21_RENDER_TYPE_NATIVE_WINDOW;

    ALOGI("NV21RenderType: %d\n", g_nv21RenderType);
