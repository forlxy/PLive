...
static int func_fill_frame(SDL_VoutOverlay* overlay, const AVFrame* frame) {
    ...
    overlay->color_space = frame->colorspace;
    overlay->color_range = frame->color_range;
    overlay->rotation = frame->angle;
    ...
    if (use_linked_frame) {
        // do nothing
                                 ...
                                 frame->format, (const uint8_t**) frame->data, frame->linesize, overlay->color_space)) {
...
static void func_screen_shot(SDL_VoutOverlay* overlay, int stride, void* dst) {
    enum AVPixelFormat src_format = AV_PIX_FMT_NONE;
    ...
    switch (overlay->format) {
        ...
        case SDL_FCC_I420:
            ...
            break;
        case SDL_FCC_RV32:
            src_format = AV_PIX_FMT_0BGR32;
            break;
        case SDL_FCC_RV24:
            src_format = AV_PIX_FMT_RGB24;
            break;
        case SDL_FCC_RV16:
            src_format = AV_PIX_FMT_RGB565;
            break;
        case SDL_FCC_NV21:
            src_format = AV_PIX_FMT_NV21;
            break;
        default:
            ALOGE("func_screen_shot: unexpected overlay format %s(%d)",
                  (char*)&overlay->format, overlay->format);
            return;
    }
...
#if defined(__ANDROID__)
    SDL_VoutOverlay_Opaque* opaque = overlay->opaque;
    // only support I420/YV12 now
    if (src_format == AV_PIX_FMT_YUV420P && opaque->linked_frame != NULL) {
        AVFrame* frame = opaque->linked_frame;

        I420ToRGB565(frame->data[0], frame->linesize[0],
...
SDL_VoutOverlay* SDL_VoutFFmpeg_CreateOverlay(int width, int height, int frame_format, SDL_Vout* display) {
    ...
    overlay->func_screen_shot   = func_screen_shot;
