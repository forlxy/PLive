...
//#define DEBUG_MUTEX_DEAD_LOCK

SDL_mutex* SDL_CreateMutex(void) {
    return SDL_CreateMutex_Name(NULL);
}

SDL_mutex*  SDL_CreateMutex_Name(const char* name) {
    SDL_mutex* mutex;
...
#ifdef DEBUG_MUTEX_DEAD_LOCK
    mutex->name ? ALOGD("[SDL_LockMutex  ] %p, name:%s \n", mutex, mutex->name) : 0;
#endif
    int ret = pthread_mutex_lock(&mutex->id);
...
#ifdef DEBUG_MUTEX_DEAD_LOCK
    mutex->name ? ALOGD("[SDL_UnlockMutex  ] %p, name:%s \n", mutex, mutex->name) : 0;
#endif
    int ret = pthread_mutex_unlock(&mutex->id);
